...
public class DatabaseChangelog {
    ...
    public void clear(MongoTemplate mongoTemplate){
        ...
        mongoTemplate.getCollection(mongoTemplate.getCollectionName(Comment.class)).drop();
    }
    ...
    @ChangeSet(order = "003", id="initComment", author = "m.tronina", runAlways = true)
    public void initComment(MongoTemplate mongoTemplate){
        List<Comment> comments = Arrays.asList(
                Comment.builder()
                        .id("1")
                        .comment("превосходно")
                        .book(BOOK_LIST.stream().filter(book -> book.getId().equals("7")).findFirst().orElse(null))
                        .build(),
                Comment.builder()
                        .id("2")
                        .comment("нормально")
                ...
                Comment.builder()
                        .id("4")
                        .comment("превосходно")
                        .book(BOOK_LIST.stream().filter(book -> book.getId().equals("1")).findFirst().orElse(null))
                        .build(),
                Comment.builder()
                        .id("5")
                        .comment("отлично")
